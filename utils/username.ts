...
let default_user_create = async (userid: string, username: string) => {
  let data = await fetch(
    `https://dok-community-tracker.vercel.appapi/profiles/empty_user_create`,
    {
...
let profile_established_boolean = async (userid: string) => {
  let data = await fetch(
    `https://dok-community-tracker.vercel.appapi/profiles/profile_established`,
    {
...
let postUsername = async (username: string, userid: string) => {
  let data = await fetch(`https://dok-community-tracker.vercel.appapi/profiles/by_username`, {
    body: JSON.stringify({ userid: userid, username: username }),
    method: "POST",
    headers: {
      "Content-type": "application/json; charset=UTF-8",
